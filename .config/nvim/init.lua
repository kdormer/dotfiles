...
-- disable netrw (despite much effort, it sucks. use nvim-tree instead)
...
require("lazy").setup({
	...
	-- automatically check for plugin updates but don't give me an annoying notification every time
...
-- Use ripgrep for greeping if it exists, fallback to ag if not and GNU grep
-- (ugh) if we don't even have that (God forbid...)
...
-- Automatically set wrap and textwidth in markdown files
...
-- Jump to global marks A S D F G with leader 1-5
map("n", "<leader>1", "`A", { desc = "Jump to mark A" })
map("n", "<leader>2", "`S", { desc = "Jump to mark S" })
map("n", "<leader>3", "`D", { desc = "Jump to mark D" })
map("n", "<leader>4", "`F", { desc = "Jump to mark F" })
map("n", "<leader>5", "`G", { desc = "Jump to mark G" })

-- Set global marks A S D F G with leader Shift+1-5
map("n", "<leader>!", "mA", { desc = "Set mark A" })
map("n", "<leader>@", "mS", { desc = "Set mark S" })
map("n", "<leader>#", "mD", { desc = "Set mark D" })
map("n", "<leader>$", "mF", { desc = "Set mark F" })
map("n", "<leader>%", "mG", { desc = "Set mark G" })
...
-- end
--
-- vim.keymap.set({ "n", "v"}, "H", with_no_scrolloff("H"))
-- vim.keymap.set({ "n", "v" }, "L", with_no_scrolloff("L"))
-- vim.keymap.set({ "n", "v" }, "M", with_no_scrolloff("M"))

-- -- =========================================================
-- -- FILE BROWSER (NETRW) - rest in peace sweet prince :(
-- -- =========================================================
--
...
-- =========================================================
-- FUZZY FINDING (BUILT-IN) (RIP, fzf-lua was just too damn good!)
-- =========================================================
...
-- Open list of buffers ready for navigating
map("n", "<leader>l", ":ls<CR>:b ", { noremap = true, silent = false })
...
map("n", "<leader>qf", toggle_quickfix, { desc = "Toggle quickfix list" })
map("n", "<leader>ql", toggle_loclist, { desc = "Toggle location list" })
...
-- 		map("n", "<leader>k", vim.lsp.buf.signature_help, { buffer = buf, desc = "Signature help" })
--
-- 		-- Actions
...
-- 		map("n", "<leader>d", vim.diagnostic.open_float, { buffer = buf, desc = "Show diagnostic" })
-- 		map("n", "<leader>q", vim.diagnostic.setloclist, { buffer = buf, desc = "Diagnostics to loclist" })
--
-- 		-- Enable completion for this buffer
-- 		vim.lsp.completion.enable(true, args.data.client_id, buf, {
-- 			autotrigger = false, -- don't trigger automatically when typing
-- 		})
-- 	end,
-- })
--
...
--
-- -- Completion navigation (in insert mode when menu is open)
--
-- map("i", "<C-n>", function()
...
--
-- map("i", "<CR>", function()
-- 	-- Only confirm if completion menu is visible
-- 	if vim.fn.pumvisible() == 1 then
-- 		return vim.api.nvim_feedkeys(vim.api.nvim_replace_termcodes("<C-y>", true, false, true), "n", false)
-- 	else
-- 		return vim.api.nvim_feedkeys(vim.api.nvim_replace_termcodes("<CR>", true, false, true), "n", false)
-- 	end
-- end, { desc = "Confirm completion or newline" })
-- map("i", "<C-e>", "<C-e>", { desc = "Abort completion" }) -- built-in dismiss
--
-- map("i", "<Tab>", function()
...
-- NOTES
-- =========================================================
-- get runtime path: :lua print(vim.inspect(vim.api.nvim_list_runtime_paths()))
